...
def assert_required_models_available(args):
    ...
    return {"pflow": model_path, "vocoder": vocoder_path}
...
def load_pflow(model_name, checkpoint_path, device):
...
def cli():
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="pflow_ljspeech",
        help="Model to use",
        choices=PFLOW_URLS.keys(),
    )
    ...
    if args.checkpoint_path is not None:
        ...
        paths["pflow"] = args.checkpoint_path
        args.model = "custom_model"

    model = load_pflow(args.model, paths["pflow"], device)
...
def batched_synthesis(args, device, model, vocoder, denoiser, texts, spk):
    ...
    for i, batch in enumerate(dataloader):
        ...
        print(f"[-Batch: {i}] PFlow-TTS RTF: {output['rtf']:.4f}")
        print(f"[-Batch: {i}] PFlow-TTS + VOCODER RTF: {rtf_w:.4f}")
    ...
    print("".join(["="] * 100))
    print(f"[] Average PFlow-TTS RTF: {np.mean(total_rtf):.4f} ± {np.std(total_rtf)}")
    print(f"[] Average PFlow-TTS + VOCODER RTF: {np.mean(total_rtf_w):.4f} ± {np.std(total_rtf_w)}")
    print("[] Enjoy the freshly whisked  PFlow-TTS!")
...
def unbatched_synthesis(args, device, model, vocoder, denoiser, texts, spk):
    ...
    for i, text in enumerate(texts):
        ...
        t = (dt.datetime.now() - start_t).total_seconds()
        rtf_w = t * 22050 / (output["waveform"].shape[-1])
        print(f"[-{i}] PFlow-TTS RTF: {output['rtf']:.4f}")
        print(f"[-{i}] PFlow-TTS + VOCODER RTF: {rtf_w:.4f}")
        total_rtf.append(output["rtf"])
        total_rtf_w.append(rtf_w)
